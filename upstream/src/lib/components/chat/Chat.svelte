	...
	const sendMessageSocket = async (model, _messages, _history, responseMessageId, _chatId) => {
		...
		const res = await generateOpenAIChatCompletion(
			localStorage.token,
			{
				...
				// orgchat — KB selection rides inline so retrieval works on
				// the very first message before the chat row exists in DB,
				// and survives the backend kb_config write-lock that blocks
				// the durability PUT once a user message has landed.
				kb_config: $selectedKBConfig?.length ? $selectedKBConfig : undefined,
