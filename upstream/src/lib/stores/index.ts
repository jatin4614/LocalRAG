...
// orgchat — KB selection for the active chat. Set by KBPickerModal (via
// Navbar.svelte) before the first message; consumed by Chat.svelte and
// passed inline as kb_config in the chat-completions body, so retrieval
// works even before the chat row exists in the DB. Resets on navigation
// to a new chat.
export type KBConfigEntry = { kb_id: number; subtag_ids: number[] };
export const selectedKBConfig: Writable<KBConfigEntry[]> = writable([]);
