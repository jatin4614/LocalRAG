...
## 10. Follow-up — global-intent comparative-query gap

Surfaced 2026-05-04 during validation of the simplified plan (after
operator pushback that the original 13-task plan was over-engineered
because vector search alone handles entity matching when
`entity_text_filter` is off).

### What we found

Three validation queries were run against the no-text-filter pipeline:

| Query shape | Result |
|---|---|
| Single-entity ("5 PoK Bde April 2026 activities") | ✅ 20 chunks, all relevant |
| Long-form paraphrase ("75 Infantry Brigade and 32 Infantry Brigade in early 2026") | ✅ 19 chunks, cross-month coverage |
| Comparative ("Compare 80 Inf Bde and 5 PoK Bde operational status in March-April 2026") | ❌ 8 chunks, all `level=doc` summaries, all about 75 Inf Bde |

The comparative query failed even though the corpus has 80 Inf Bde and
5 PoK Bde content. Diagnosis:

1. The query was classified as **`global` intent** by the QU LLM (the
   "compare ... operational status in March-April" phrasing matches the
   `global:overview_of` / `global:state_of` patterns plus the QU
   comparison-verb predicate).
2. The `global` intent path **skips multi-entity decompose entirely**
   and queries the doc-summary index (`level=doc` only).
3. The doc-summary points were **auto-generated by the LLM at ingest
   time**. Because 75 Inf Bde is the dominant brigade in this corpus,
   the auto-summariser foregrounded 75 Inf Bde in every monthly summary
   — so all eight retrieved doc-summaries describe 75 Inf Bde even when
   the user asked about a different brigade.

### Why this is orthogonal to the work in §4-§6

`entity_text_filter`, the synonym table, and the per-entity rerank quota
all live in the **multi-entity decompose path** (Phase 6.X). The global
intent path doesn't touch any of them. So this failure mode would have
shown up regardless of whether `entity_text_filter` is on or off.

### Three candidate fixes (all out of scope for this spec)

- **A. Re-summarise per-doc with explicit entity coverage.** During
  ingest, when generating the per-doc summary, prompt the LLM to
  enumerate every named entity (brigade, unit, location) appearing
  in the doc rather than narrate the dominant story. Trade-off: longer
  summaries (~2× length, ~2× cost), more uniform coverage across
  comparative queries. ~M complexity, ingest-side only, requires
  re-summarisation of every existing doc.
- **B. Route comparative queries away from `global` intent.** Tighten
  the QU intent classifier so a query that names ≥2 entities AND uses
  a comparison verb routes to `specific` (multi-entity decompose) even
  if it also mentions an aggregation period. ~S complexity, classifier-
  side only. Risk: shifts other comparative-aggregation queries into
  the per-chunk path which may starve the LLM of high-level summary
  context.
- **C. Hybrid global+specific retrieval for comparative queries.**
  When the query is comparative AND names ≥2 entities, run BOTH the
  global summary retrieval AND a multi-entity decompose, merge with
  per-entity quota. ~M complexity, retrieval-side. Trade-off: 2× retrieve
  cost on comparative queries.

Recommendation: **B** is the smallest fix and most likely to be enough
on its own. **A** is the "structurally right" answer but is a bigger
ingest-side investment. Defer until an operator surfaces a comparative
query that **B** alone can't solve.

### Tracking

This is a separate spec / plan. Not bundled into the 2026-05-03 work.
File a follow-up issue with title "Comparative multi-entity queries
get global intent → biased doc summaries" and link this section.
