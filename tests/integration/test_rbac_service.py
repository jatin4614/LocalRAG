...
async def test_rbac_direct_and_group_grants(session):
    # ``get_allowed_kb_ids`` queries upstream's "user" / group_member
    # tables which the conftest exposes as VIEWs over users /
    # user_groups, casting ids to text. Pass user_id as str to match
    # the UUID-string contract (id="1" rather than id=1).
    ...
    # kb_access.user_id is VARCHAR(255), group_id is TEXT
    await session.execute(text(
